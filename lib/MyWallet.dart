...
import 'package:bip39/bip39.dart'
    as bip39; //third-party library. NOT PART OF DART-SV
import 'package:hex/hex.dart';
import 'package:sprintf/sprintf.dart';
//import 'package:cli_example/BitIndex.dart';

class MyWallet {
  late HDPrivateKey _hdMaster;

  late SVPrivateKey _walletPrivKey, _walletPrivKey2;
  late SVPublicKey _walletPubKey, _walletPubKey2;
  late Address _spendingAddress,
      _receivingAddress; //we will spend money from _address to _address2
  ...
  MyWallet.fromSeed(String mnemonic) {
    ...
    HDPrivateKey xprivateKey = HDPrivateKey.fromXpriv(this._hdMaster.xprivkey)
        .deriveChildKey("m/44'/1'/1'/0'");
    ...
    HDPrivateKey xprivateKey2 = HDPrivateKey.fromXpriv(this._hdMaster.xprivkey)
        .deriveChildKey("m/44'/1'/1'/1'");
    ...
    this._walletPubKey2 =
        SVPublicKey.fromHex(HEX.encode(xpublicKey2.keyBuffer));

    _receivingAddress =
        this._walletPrivKey.toAddress(networkType: NetworkType.TEST);
    _spendingAddress =
        this._walletPrivKey2.toAddress(networkType: NetworkType.TEST);
  }

  String createWalletTxn(Address address,
      List<TransactionOutput> utxosToSpendFrom, BigInt amount) {
    var transaction = new Transaction();

    var unlockBuilder = P2PKHUnlockBuilder(this._walletPrivKey.publicKey);
    utxosToSpendFrom.forEach((utxo) => transaction.spendFromOutput(
        utxo, Transaction.NLOCKTIME_MAX_VALUE,
    ...
    transaction
        ...
        .sendChangeTo(_receivingAddress,
            scriptBuilder: changeLockBuilder) // spend change to myself
        .withFeePerKb(1000);

    transaction.signInput(0, this._walletPrivKey,
